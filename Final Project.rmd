...
hist(transformed_gene_data[["ENSG00000142794"]], main="transformed ENSG0000014279 Gene Expression Distribution(1/sqrt(ENSG0000014279+2)", ylab="Frequency", xlab="Transformed Gene Expression Value")
...
```{r stats Selection}
cbind(selected_p_stats,selected_stats,forward_bic_stats)
car::vif(selected)
...
# Particulary points 283, 309, and 317 have a high cook's distance.
plot(model, which=1)
plot(model, which=2)
plot(model, which=3)
plot(model, which=4)
...
coefficients <- rbind(model$coefficients ,new_model$coefficients,new_model_betas$coefficients, leverage_model$coefficients)
...
add_to_coefficients_data <- function(model, model_data){
  ...
  current_model_stats <- cbind(sqrt(MSE), BIC(current_model), summary(current_model)$adj.r.squared, DAAG::press(current_model))
...
summary_of_excluding_points

# We only would consider dropping HighDF Betas since dropping the other elements hurts BIC. We choose not to drop the points. The coefficients are not changed greatly enough to warrant us
# to exclude so many points that contain valuable variation from the overall population.
```
...
DAAG::press(model)
DAAG::press(validation_model_one)
